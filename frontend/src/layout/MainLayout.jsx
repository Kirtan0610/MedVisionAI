...
export default function MainLayout() {
  ...
  const avatar   = "bg-blue-600";
  const divider  = dark ? "border-slate-800" : "border-slate-200";
  const mutedTxt = dark ? "text-slate-500" : "text-slate-400";
    ...
    <div className={`flex min-h-screen ${bg} transition-colors duration-300`} style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      `}>
        ...
        <div className={`flex items-center gap-3 px-5 h-16 border-b ${divider} shrink-0`}>
          <div className={`w-8 h-8 rounded-lg ${avatar} flex items-center justify-center text-white font-bold text-sm shrink-0`}>M</div>
          <div>
            ...
            <p className={`text-[0.6rem] uppercase tracking-widest font-semibold ${mutedTxt}`}>AI Health</p>
          </div>
          <button onClick={() => setOpen(false)} className={`ml-auto lg:hidden p-1 rounded ${mutedTxt} hover:text-white`}>✕</button>
        </div>

        {/* User info */}
        <div className={`mx-3 mt-4 p-3 rounded-xl border ${dark ? "border-slate-800 bg-slate-800/50" : "border-slate-100 bg-slate-50"}`}>
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg ${avatar} flex items-center justify-center text-white font-bold text-sm shrink-0`}>{initial}</div>
            <div className="min-w-0">
              <p className={`text-sm font-semibold truncate ${dark ? "text-slate-200" : "text-slate-800"}`}>{user?.name || "User"}</p>
              <p className={`text-[0.65rem] font-medium ${mutedTxt}`}>Patient Account</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5">
          <p className={`text-[0.6rem] font-bold uppercase tracking-widest px-2 mb-2 ${mutedTxt}`}>Navigation</p>
          {NAV.map(({ path, label, icon }) => {
            ...
            return (
              <Link key={path} to={path} onClick={() => setOpen(false)}
                ...
                <span className={`text-base w-5 text-center shrink-0 font-mono ${active ? "text-white" : ""}`}>{icon}</span>
                {label}
        ...
        <div className={`p-3 border-t ${divider} space-y-2`}>
          ...
          <button onClick={() => { logout(); navigate("/login"); }}
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-semibold border
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className={`lg:hidden sticky top-0 z-20 flex items-center gap-3 px-4 h-14 border-b ${topbar}`}>
          <button onClick={() => setOpen(true)} className={`p-2 rounded-lg transition-colors ${dark ? "hover:bg-slate-800" : "hover:bg-slate-100"}`}>
